...
app.post('/login', async (req, res) => {
    ...
    else res.json({ success: false, message: "아이디 또는 비번이 틀렸습니다." });
});
...
app.post('/ask-ai', async (req, res) => {
    try {
        ...
        const completion = await groq.chat.completions.create({
            messages: [
                { role: "system", content: "너는 친절한 Qwen AI 챗봇이야. 한국어로 짧고 명확하게 대답해줘." },
        ...
        res.json({ success: false, message: "AI가 응답하지 않습니다." });
    }
});

// 프로필 및 친구 관리 API (생략 없이 통합)
